...
# Attempt to load the pre-baked Cython core
try:
    import catsnescore_0_1 as nes_core
    CORE_LOADED = True
    CORE_NAME = "Cat'snescore 0.1 (Cython)"
except ImportError:
    try:
        import gemininesemu0_1 as nes_core
        CORE_LOADED = True
        CORE_NAME = "gemininesemu0_1 (Cython)"
    except ImportError:
        CORE_LOADED = False
    ...
    class PyNesCore:
        ...
        def _write_mapper(self, addr, value):
            ...
            if self.mapper == 1:
                ...
                if commit:
                    ...
                    if reg == 0:
                        self.mmc1_ctrl = data
                        # Bits 0-1 of MMC1 control set mirroring:
                        #   0 = single-screen lower, 1 = single-screen upper,
                        #   2 = vertical-mirror (iNES "horizontal"),
                        #   3 = horizontal-mirror (iNES "vertical")
                        m = data & 0x03
        ...
        def _write(self, addr, value):
            ...
            if addr == 0x4014:
                ...
                # 513 or 514 cycles depending on CPU cycle parity.
                self.cycles += 513 + (self.cycles & 1)
                return
            if 0x4000 <= addr < 0x4020:
                # APU register activity marker for optional UI beep.
